...
def main(seed):
    ...
    training_args = TrainingArguments(
        output_dir=f"/network/scratch/t/tengmeli/mask2former_fine_tuned_dsm_balsam_large/resize_50epochs_large_dsm_neurips_{seed}",
        per_device_train_batch_size=4, # Adjust based on your GPU memory
        per_device_eval_batch_size=4,
        num_train_epochs=50, # Or more, depending on dataset size and convergence
        learning_rate=1e-4,
        warmup_steps=1000,
        weight_decay=0.01,
        logging_dir=f"/network/scratch/t/tengmeli/mask2former_fine_tuned_dsm_balsam_large/resize_50epochs_large_dsm_neurips_{seed}",
        logging_steps=100,
        save_steps=1000,
        save_total_limit=4,
        #eval_strategy="epoch",
