...
processor = Mask2FormerImageProcessor.from_pretrained(model_name, num_labels =9, do_reduce_labels=True) #,do_resize=False)
...
model_path ="/network/scratch/t/tengmeli/mask2former_fine_tuned_dsm_balsam/resized_dsm_42/checkpoint-196800"# "/network/scratch/t/tengmeli/mask2former_fine_tuned_dsm_balsam/not_resized_dsm/checkpoint-338000"
...
try:
    ...
    print("Model state_dict loaded manually with strict=True.")
except RuntimeError as e:
    print(f"Error loading state_dict manually with strict=True: {e}")
    print("Attempting to load with strict=False.")
    model.load_state_dict(state_dict, strict=False)
    print("Model state_dict loaded manually with strict=False. Be aware of potential uninitialized layers.")
...
test_dataset = TreesDSMDataset("/network/projects/trees-co2/final_tiles/merged_annots_dsm_test_new_maskrcnn.json", 
               dsm_normalization="max",
...
train_dataloader = DataLoader(test_dataset, batch_size=1, shuffle=False, collate_fn=data_collator)
    ...
    for i, elem in enumerate(train_dataloader):
        ...
        for k, v in elem.items():
            if isinstance(v, list):
                # If the value is a list, iterate through its elements and move each to device
                inputs_on_device[k] = [u.to(device) if isinstance(u, torch.Tensor) else u for u in v]
            elif isinstance(v, torch.Tensor):
                # If the value is a single tensor, move it to the device
                inputs_on_device[k] = v.to(device)
            else:
                # For other types (strings, ints, etc.), keep them as is
                inputs_on_device[k] = v  
        outputs = model(**inputs_on_device)
        predictions = processor.post_process_instance_segmentation(outputs)
        
        min_val_for_masks = 0 # Starting value for which we want a mask
        max_val_in_T = predictions[0]["segmentation"].max().item()
        num_masks = max_val_in_T - min_val_for_masks + 1
        comparison_values = torch.arange(num_masks)
        comparison_values_reshaped = comparison_values[:, None, None]
        boolean_tensor = (predictions[0]["segmentation"]== comparison_values_reshaped)
        scores =[a["score"] for a in  predictions[0]["segments_info"]]
        labels =[a["label_id"] for a in  predictions[0]["segments_info"]]
        
        boolean_tensor_gt =elem['mask_labels'][0].type(torch.bool)
        map_metric.update(
                    [
                        {
                            "labels": torch.LongTensor(labels),
                            "masks": boolean_tensor,
                            "scores": torch.FloatTensor(scores)
                        }
                    ],
                    [{"labels":elem["class_labels"][0], 
                      "masks": boolean_tensor_gt
        ...
        map_metric_single_class.update(
            [
                {
                    "labels": torch.zeros(torch.LongTensor(labels).shape, dtype=torch.int),
                    "masks": boolean_tensor,
                    "scores":  torch.FloatTensor(scores),
                }
            ],
            [{"labels":torch.zeros(len(elem["class_labels"][0]), dtype=torch.int),
            "masks": boolean_tensor_gt
                        
                    }]
        )
        img_to_label_dict[i] = [0 for i in range(len(elem["class_labels"][0]))] #list(
                        #elem["class_labels"][0].cpu().numpy()
                    #)
        for idxt, t in enumerate(boolean_tensor_gt.cpu().numpy()):
            max_iou = 0

            for idx, p in enumerate(boolean_tensor.cpu().numpy()):
                intersection = (t & p).sum().astype(np.float32)
                union = (t | p).sum().astype(np.float32)
...
print("iou", np.mean(iou_per_instance))
            

map_score = map_metric.compute()
map_single_score = map_metric_single_class.compute()

print("map_single", map_single_score)
print("map", map_score)

print("map_per_class", map_score["map_per_class"])
print("map", map_score["map"])
print("map_single", map_single_score["map"])
iou_list = []
if "ious" in map_single_score:
...
print("iou", total_iou / num_instances)
