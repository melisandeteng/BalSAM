...
class TreesDataset(SegmentationLabeledRasterCocoDataset):
    ...
    def __getitem__(self, idx: int):
        ...
        tile = tile / 255
        # tile = (tile - mean)/std #tv_tensors.Image(tile/255,  dtype=torch.float)
...
class TreesDSMDataset:
    ...
    def __getitem__(self, idx: int):
        ...
        ]  # data_im["annotations"]#[d for d in self.data["annotations"] if d.get('image_id') ==data_im["id"]]
        ...
        dsm = normalize_dsm(
            dsm, mode=self.dsm_normalization
        )  # ,  means_dsm= self.means_dsm, stds_dsm= self.stds_dsm)
