...
def get_dataloader(config, fold, collate_fn=collate_fn_detection):
    if fold == "train":
        transform = a.Compose(
...
def train(config_path="config.yaml", cli_config=None, scaler=None):
    ...
    if config.wandb:
        ...
        # config.save_ckpt_path = config.save_ckpt_path + f"{wandb_id}_{config.seed}"
        wandb.init(
            dir=config.log_dir,
    ...
    train_dataloader = get_dataloader(config, "train", collate_fn_detection)
    val_dataloader = get_dataloader(config, "val", collate_fn_detection)
    test_dataloader = get_dataloader(config, "test", collate_fn_detection)
    ...
    for epoch in range(epoch_start, config.num_epochs):
        ...
        if (epoch + 1) % config.save_every_epochs == 0:
            torch.save(
                ...
                os.path.join(config.log_dir, f"fasterrcnn_epoch_{epoch}.pt"),
            )
