...
def get_inv_frequencies(loader):

    weights = {}

    for index in loader.dataset.tiles:
        for elem in loader.dataset.tiles[index]["labels"]:
            if elem["category_id"] not in weights:
                weights[elem["category_id"]] = 1
            else:
                weights[elem["category_id"]] += 1
    total = sum(weights.values())
    freqs = {k: total / v for (k, v) in weights.items()}
    max_val = max(freqs.values())
    freqs = {k: 10 * v / max_val for (k, v) in freqs.items()}
    sorted_dict = dict(sorted(freqs.items()))
    return sorted_dict
