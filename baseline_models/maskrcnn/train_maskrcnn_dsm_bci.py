...
import utils
from dataset import TreesDSMBCIDataset
...
def get_dataloader(config, fold, collate_fn=collate_fn_segmentation):
    if fold == "train":
        transform = a.Compose(
    ...
    dataset = TreesDSMBCIDataset(fold, Path(config.root_path), transform=transform)
    if fold == "train" and config.percentage != 1:
...
def train(config_path="config.yaml", cli_config=None, scaler=None):
    ...
    if not os.path.isdir(config.save_ckpt_path):
        ...
        if len(os.listdir(config.save_ckpt_path)) != 0:
    ...
    train_dataloader = get_dataloader(config, "train", collate_fn_segmentation)
    val_dataloader = get_dataloader(config, "val", collate_fn_segmentation)
    test_dataloader = get_dataloader(config, "test", collate_fn_segmentation)
    ...
    for epoch in range(epoch_start, config.num_epochs):
        ...
        for images, targets in tqdm(train_dataloader):
            images = images.clone().to(
                device
            )  # list(image.to(device) for image in images)
            targets = [
                {
        ...
        with torch.no_grad():
            ...
            for i, (im, tar) in tqdm(enumerate(val_dataloader)):
                ...
                for idx, outputs in enumerate(out):
                    # print("scores", outputs["scores"].mean(), outputs["scores"].min(), outputs["scores"].max())
